...
void setup_timer() {
    ...
    // ms -> ticks
    tick_shift = find_reciprocal_shift(timebase_frequency, 1000);
    tick_mult  = ((__uint128_t)timebase_frequency << tick_shift) / 1000;
...
void timer_set_timeout_ms(int ms) {
    ...
    uint64_t next_time = curr_time + (((__uint128_t)ms * tick_mult) >> tick_shift);
